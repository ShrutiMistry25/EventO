import type { User, Venue, Booking, ChatMessage, TimeSlot, VenueAvailability } from '../types';
...
const VENUE_AVAILABILITY_KEY = 'venuebook_venue_availability';
...
export const getVenueAvailability = (): VenueAvailability[] => {
  const data = localStorage.getItem(VENUE_AVAILABILITY_KEY);
  return data ? JSON.parse(data) : [];
};

export const saveVenueAvailability = (availability: VenueAvailability[]): void => {
  localStorage.setItem(VENUE_AVAILABILITY_KEY, JSON.stringify(availability));
};

export const addVenueAvailability = (availabilityItem: VenueAvailability): void => {
  const availability = getVenueAvailability();
  availability.push(availabilityItem);
  saveVenueAvailability(availability);
};

export const getVenueAvailabilityByVenueId = (venueId: string): VenueAvailability[] => {
  const availability = getVenueAvailability();
  return availability.filter((item) => item.venueId === venueId);
};
